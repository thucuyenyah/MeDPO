...
def load_dataset_with_cache_recovery(*args, **kwargs):
    """Load a Hugging Face dataset and retry once with a fresh download if the cached metadata is stale."""
    try:
        return datasets.load_dataset(*args, **kwargs)
    except Exception as exc:
        message = str(exc)
        recoverable = (
            "Invalid pattern: '**' can only be an entire path component" in message
            or "LocalFileSystem is not supported" in message
            or "must be called with a dataclass type or instance" in message
        )
        if not recoverable:
            raise
        ...
        print(f"Retrying dataset load for {dataset_name} split={split} with force_redownload due to stale cache: {exc}")
        cache_dir = kwargs.get("cache_dir")
        if cache_dir is not None:
            dataset_cache_prefix = dataset_name.replace("/", "___")
            cache_root = os.path.join(cache_dir, dataset_cache_prefix)
            if os.path.exists(cache_root):
                print(f"Removing stale dataset cache at {cache_root}")
                shutil.rmtree(cache_root, ignore_errors=True)
        retry_kwargs = dict(kwargs)
        retry_kwargs["download_mode"] = "force_redownload"
        return datasets.load_dataset(*args, **retry_kwargs)
...
def get_se(split, silent=False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    dataset = load_dataset_with_cache_recovery('HuggingFaceH4/stack-exchange-preferences', cache_dir=cache_dir)['train']
    print('done')
...
def get_pku(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    dataset = load_dataset_with_cache_recovery('PKU-Alignment/PKU-SafeRLHF', split=split, cache_dir=cache_dir)
    print('done')
...
def get_shp(split: str, tokenizer, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    dataset = load_dataset_with_cache_recovery('stanfordnlp/SHP', split=split, cache_dir=cache_dir)
    print('done')

    data = defaultdict(lambda: defaultdict(list))
    for row in tqdm.tqdm(dataset, desc='Processing SHP', disable=silent):
        prompt = '\n\nHuman: ' + row['history'] + '\n\nAssistant:'
        respA = clean_response(row['human_ref_A'], tokenizer)
        respB = clean_response(row['human_ref_B'], tokenizer)
...
def get_hh(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    dataset = load_dataset_with_cache_recovery('Anthropic/hh-rlhf', split=split, cache_dir=cache_dir)
    print('done')
...
def get_ultrabin(split, tokenizer, silent=False, cache_dir=None):
    split = "train_prefs" if split == "train" else "test_prefs"
    print(f"Loading UltraFeedback binarized ({split})...")
    dataset = load_dataset_with_cache_recovery(
    ...
    for row in dataset:
        raw_prompt = clean_prompt(row["prompt"], tokenizer)

        prompt = "\n\nHuman: " + raw_prompt + "\n\nAssistant:"
        chosen = " " + clean_response(row["chosen"][-1]["content"], tokenizer)
        rejected = " " + clean_response(row["rejected"][-1]["content"], tokenizer)


        n = len(data[prompt]["responses"])
        data[prompt]["responses"].extend([chosen, rejected])
        data[prompt]["pairs"].append((n, n + 1))
        data[prompt]["sft_target"] = chosen

    return data


def get_ultrafeedback_armorm(split, tokenizer, silent=False, cache_dir=None):
    print(f"Loading ultrafeedback_armorm ({split})...")
    dataset = load_dataset_with_cache_recovery(
    ...
    for row in dataset:
        ...
        # chosen / rejected là list các turn → lấy assistant cuối
        chosen = " " + clean_response(row["chosen"][-1]["content"], tokenizer)
        rejected = " " + clean_response(row["rejected"][-1]["content"], tokenizer)
...
def get_ultrafeedback_armorm_gemma(split, tokenizer, silent=False, cache_dir=None):
    print(f"Loading ultrafeedback_armorm_gemma ({split})...")
    dataset = load_dataset_with_cache_recovery(
...
def get_dataset(name: str, split: str,tokenizer, silent: bool = False, cache_dir: str = None):
    """Load the given dataset by name. Supported by default are 'shp', 'hh', and 'se'."""
    if name == 'shp':
        data = get_shp(split, tokenizer, silent=silent, cache_dir=cache_dir)
    elif name == 'hh':
        ...
        data = get_ultrabin(split,tokenizer, silent=silent, cache_dir=cache_dir)
    elif name == 'ultrallama':
        data = get_ultrafeedback_armorm(
            split, tokenizer, silent=silent, cache_dir=cache_dir
        )
    elif name == 'ultragemma':
        data = get_ultrafeedback_armorm_gemma(
            split, tokenizer, silent=silent, cache_dir=cache_dir
        )
    else:
...
def get_batch_iterator(names: List[str],
    ...
    with TemporarilySeededRandom(seed):
        ...
        for name in names:
            ...
            for prompt, data in get_dataset(name, split, tokenizer, silent=silent, cache_dir=cache_dir).items():
