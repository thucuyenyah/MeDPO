...
def resolve_torch_device(device: Union[str, int]) -> str:
    """Resolve trainer device hints into a concrete torch device string."""
    if isinstance(device, int):
        return f"cuda:{device}" if torch.cuda.is_available() else "cpu"
    return device


def slice_and_move_batch_for_device(batch: Dict, rank: int, world_size: int, device: Union[str, int]) -> Dict:
    ...
    on_device = {k: (v.to(resolved_device) if isinstance(v, torch.Tensor) else v) for k, v in sliced.items()}
    return on_device
